...
#Slope
                self.m = -((self.gx-self.x)/(self.gy-self.y))
#Y-intercept
                self.b = ((self.y+self.gy)/2)-self.m*((self.x+self.gx)/2)
#Random distance
                while True:
                    self.dis = random.randint(int(-(((self.x-self.gx)**2+(self.y-self.gy)**2)**(1/float(2)))/2), int((((self.x-self.gx)**2+(self.y-self.gy)**2)**(1/float(2)))/2))
#random x guess2
                    self.gx2 = (self.x+self.gx)/2+self.dis*cos(self.u+pi/2)
#random y guess2
                    self.gy2 = (self.y+self.gy)/2+self.dis*sin(self.u+pi/2)
                ...
                print(self.gx2)
                print(self.gy2)
                canvas.create_rectangle(self.gx2,self.gy2,self.gx2+2,self.gy2+2)
#atan2 of second delta points
                self.u2 = atan2(self.gy2-self.y,self.gx2-self.x)
            elif ((self.x-self.gx2)**2+(self.y-self.gy2)**2)**(1/float(2))<spc:
#                self.u2 = self.u
                self.u2 = atan2(self.gy-self.y,self.gx-self.x)
                print('1')
                print(self.u)
                print(self.gx)
                print(self.gy)
                canvas.create_rectangle(self.gx,self.gy,self.gx+2,self.gy+2,outline='orange')
            self.xa = self.x+spc*cos(self.u2)
            self.xb = self.y+spc*sin(self.u2)
