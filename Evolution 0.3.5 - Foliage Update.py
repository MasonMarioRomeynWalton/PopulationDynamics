...
for d in range(0, f):
    ...
    p = fm[d]*s+(s/2)
    q = fn[d]*s+(s/2)
    canvas.create_rectangle(p-o, q-o, p+o, q+o, fill='lime', tags=('food', ('f'+str(d))))
v = 0
yy = []
xx = []
i = 0
def move():
    ...
    for br in range (0, ge):
        for bb in range (0,ge):
            if xx[br] == xx[bb] and yy[br] == yy[bb] and br != bb and br < bb and xx[br] > 0:
                if l[br] == l[bb]and dm[br] < hl and dm[bb] < hl:
                    l.append('')
                    if l[br] == 1:
                        l[e] = 1
                    if l[br] == 2:
                    ...
                    e = e+1
    plb = []
    for itc in range (0, e):
        canvas.itemconfig('d' + str(itc), outline='black', width=1)

        plb.append(0)
        
    for itf in range (0, f):
        canvas.itemconfig('f' + str(itf), outline='black', width=1)
    for fo in range (0, f):
        for cr in range (0, e):
            if fm[fo] == xx[cr] and fn[fo] == yy[cr] and xx[cr] > 0:
                if l[cr] == 1:
                    canvas.itemconfig('d' + str(cr), outline='lime', width=3)
                    canvas.lift('d' + str(cr))
                    plb[cr] = 1
                if l[cr] == 2:
    ...
    for fr in range (0, f):
        widef.append(1)
        for fb in range (0, f):
            if fm[fr] == fm[fb] and fn[fr] == fn[fb] and fr != fb and fm[fr] > 0:
                widef[fr] = widef[fr] + 2
                canvas.itemconfig('f' + str(fr), outline='#008000', width=widef[fr])                
    wide=[]
    for nr in range (0, e):
        wide.append(1)
        for nb in range (0, e):
            if xx[nr] == xx[nb] and yy[nr] == yy[nb] and nr != nb and xx[nr] > 0:
                if l[nr] == 1 and l[nb] == 2:
                    ...
                    yy[nb] = 0
                    print('kill')
                if l[nr] == 1 and l[nb] == 1:
                    wide[nr] = wide[nr] + 1
                    if plb[nr] == 0 and plb[nb] == 0:
                        outl = '#800000'
                    if plb[nr] == 1 or plb[nb] == 1:
                        outl = '#808000'
                    canvas.itemconfig('d' + str(nr), outline=outl, width=wide[nr])
                if l[nr] == 2 and l[nb] == 2:
                    wide[nr] = wide[nr] + 1
                    canvas.itemconfig('d' + str(nr), outline='#000080', width=wide[nr])
    for de in range (0, e):
        if dm[de] == life:
